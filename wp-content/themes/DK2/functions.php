...
function shprinkone_setup() {
	...
	class ShprinkOne_Walker_Comment extends Walker_Comment {
			...
			<div class="media <?php echo join(' ', get_comment_class()); ?>"
				...
				<div class="media-body">
					<h4 class="media-heading">
						<?php printf(__('%s <span class="says">написал:</span>', 'shprinkone'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link())); ?>
					</h4>
					<?php if ($comment->comment_approved == '0') : ?>
						<em class="comment-awaiting-moderation"><?php _e('Ваш комментарий ожидает модерации.', 'shprinkone'); ?>
						</em> <br />
					<?php endif; ?>
					...
					<div class="comment-meta commentmetadata">
						<a
							href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
								...
								printf(__('%1$s в %2$s', 'shprinkone'), get_comment_date(), get_comment_time());
								?>
						</a>
		...
		function shprinkone_get_the_author_posts_link() {
			...
			$link = sprintf($format, get_author_posts_url(get_the_author_meta('ID'), get_the_author_meta('nicename')), esc_attr(sprintf(__('Опубликовано: %s', 'shprinkone'), get_the_author())), get_the_author());

			return sprintf(__('Опубликовано: %s', 'shprinkone'), $link);
		}
		...
		function shprinkone_get_post_meta($inline = false, $author = true, $date = false, $category = true, $tag = true, $comments = false, $sticky = false, $tooltip = false) {
			...
			if (is_sticky() && $sticky) {
				...
				$html .= ($tooltip) ? 'data-content="' . __('Важная статья', 'shprinkone') . '"' : '';
				$html .= '><i class = "icon-star"></i> ';
				$html .= ($tooltip) ? '' : __('Важная статья', 'shprinkone');
				$html .= '</li>';
			}
			if ($date) {
				$formatedDate = get_the_modified_date(__('M d, Y', 'shprinkone'));
				$html .= '<li class = "post-date" ';
				$html .= ($tooltip) ? 'title="' . __('Опубликовано', 'shprinkone') . '" data-content="' . $formatedDate . '"' : '';
			...
			if ($author) {
				...
				$html .= ($tooltip) ? 'title="' . __('Автор', 'shprinkone') . '" data-content="' . $authorName . '"' : '';
			...
			if ($category) {
				if (has_category()):
					...
					$html .= ($tooltip) ? 'title="' . __('Категории', 'shprinkone') . '" data-content="' . join(", ", $categoryList) . '"' : '';
					$html .= '><i class = "icon-folder-open"></i> ';
					$html .= ($tooltip) ? '' : sprintf(__('Категория: %s', 'shprinkone'), get_the_category_list(' ', '', false));
					$html .= '</li>';
				endif;
			}
			if ($tag) {
				if (has_tag()):
					...
					$html .= ($tooltip) ? 'title="' . __('Теги', 'shprinkone') . '" data-content="' . join(", ", $tagList) . '"' : '';
					$html .= '><i class = "icon-tags"></i> ';
					$html .= ($tooltip) ? '' : get_the_tag_list(__('Тег:  ', 'shprinkone'), ' ');
					$html .= '</li>';
				endif;
			}
			if ($comments) {
				...
				$html .= ($tooltip) ? 'title="' . __('Комментарии', 'shprinkone') . '" data-content="' . $commentNumber . '"' : '';
		...
		function shprinkone_get_no_result() {
			...
			$html .= '<p>' . __('Ваш поиск не дал результатов.', 'shprinkone') . '</p>';
			$html .= '</section>';
			return $html;
		}
		...
		function shprinkone_comment_form($args = array(), $post_id = null) {
			...
			$fields = array(
				'author' => '<p class="comment-form-author"><label for="author">' . __('Логин', 'shprinkone') . ( $req ? ' <span class="required">*</span>' : '' ) . '</label><input id="author" name="author" class="form-control" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
				...
				'url' => '<p class="comment-form-url"><label for="url">' . __('Веб-сайт', 'shprinkone') . '</label><input id="url" name="url" class="form-control" type="text" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" /></p>',
			);

			$required_text = sprintf(' ' . __('Обязательные поля помечены %s', 'shprinkone'), '<span class="required">*</span>');
			$defaults = array(
				'fields' => apply_filters('comment_form_default_fields', $fields),
				'comment_field' => '<p class="comment-form-comment"><label for="comment">' . __('Введите текст комментария', 'shprinkone') . '</label><textarea id="comment" name="comment" class="form-control" aria-required="true"></textarea></p>',
				...
				'logged_in_as' => '<p class="logged-in-as">' . sprintf(__('Вы зашли как <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Выйти?</a>', 'shprinkone'), get_edit_user_link(), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
				'comment_notes_before' => '<p class="comment-notes">' . __('Ваш email не увидят другие пользователи.', 'shprinkone') . ( $req ? $required_text : '' ) . '</p>',
				'comment_notes_after' => null,
				'id_form' => 'commentform',
				'id_submit' => 'submit',
				'title_reply' => __('Прокомментировать', 'shprinkone'),
				'title_reply_to' => __('Leave a Reply to %s', 'shprinkone'),
				'cancel_reply_link' => __('Cancel reply', 'shprinkone'),
				'label_submit' => __('Оставить комментарий', 'shprinkone'),
			);
