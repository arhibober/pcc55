 ...
 * @return string
 */
function wpuf_feat_img_html( $attach_id, $post_id ) {
    $image = wp_get_attachment_image_src( $attach_id, 'thumbnail' );
    $post = get_post( $attach_id );

    $html = sprintf( '<div class="wpuf-item" id="attachment-%d" style="width: 400px;">', $attach_id );
						global $wpdb;
$sqlstr = "SELECT meta_value from wp_postmeta where post_id=(select meta_value from wp_postmeta where post_id=".$post_id ." and meta_key='_thumbnail_id') and meta_key='_wp_attached_file'";
$photo_list = $wpdb->get_results($sqlstr, ARRAY_A);
//echo "<img src='http://localhost/pcc55/wp-content/uploads/".$photo_list[0]["meta_value"]."'>";
    $html .= sprintf( '<img src="http://localhost/pcc55/wp-content/uploads/'.$photo_list[0]["meta_value"].'" alt="%s"/>', esc_attr( $post->post_title ) );
    $html .= sprintf( '<a class="wpuf-del-ft-image button" href="#" data-id="%d">%s</a><br/> ', $attach_id, __( 'Удалить аватар', 'wpuf' ) );
...
class WPUF_Walker_Category_Checklist extends Walker {
    ...
    function start_el( &$output, $category, $depth = 0, $args = array(), $object_id = 0 ) {
        ...
        $output .= "\n<li id='{$taxonomy}-{$category->term_id}'$class style='list-style-type: none; !important'>" . '<label class="selectit"><input value="' . $category->term_id . '" type="checkbox" name="' . $name . '[]" id="in-' . $taxonomy . '-' . $category->term_id . '"' . checked( in_array( $category->term_id, $selected_cats ), true, false ) . disabled( empty( $args['disabled'] ), false, false ) . ' /> ' . esc_html( apply_filters( 'the_category', $category->name ) ) . '</label>';
    }
