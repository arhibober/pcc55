...
class WPUF_Edit_Post {
    ...
    function shortcode() {
        ...
        if ( is_user_logged_in() ) {
            ...
            printf( __( "Пожалуйста, войдите на сайт для просмотра этой страницы.", 'wpuf' ), wp_loginout( '', false ) );
        }
    ...
    function prepare_form() {
        ...
        //process post
        if ( isset( $_POST['wpuf_edit_post_submit1'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'wpuf-edit-post' ) ) {
            $this->submit_post();
    ...
    function edit_form( $curpost ) {
        ...
        <div id="wpuf-post-area1">
            <form name="wpuf_edit_post_form1" id="wpuf_edit_post_form1" action="" enctype="multipart/form-data" method="POST">
                ...
                <ul class="wpuf-post-form">
                    ...
                    <?php if ( $featured_image == 'yes' ) { ?>
                        <?php if ( current_theme_supports( 'post-thumbnails' ) ) { ?>
                            <li>
                                ...
                                <div id="wpuf-ft-upload-container1">
                                    <div id="wpuf-ft-upload-filelist1">
                                        <?php
                                        $style = '';
                                    ...
                                    <a id="wpuf-ft-upload-pickfiles1" class="button" href="#"><?php echo wpuf_get_option( 'ft_image_btn_label', 'wpuf_labels', 'Upload Image' ); ?></a>
                                </div>
                                <div class="clear"></div>
                            </li>
                        <?php } else { ?>
                    ...
                    <li>
                        ...
                        <input type="text" name="wpuf_post_title1" id="new-post-title1" minlength="2" value="<?php echo esc_html( $curpost->post_title ); ?>">
                        <div class="clear"></div>
                        ...
                        <li>
                            ...
                            <div class="category-wrap" style="float:left;">
                                <div id="lvl01">
                                    <?php
                                    if ( $cat_type == 'normal' ) {
                                        wp_dropdown_categories( 'show_option_none=' . __( '-- Выберите категорию --', 'wpuf' ) . '&hierarchical=1&hide_empty=0&orderby=name&name=category1[]&id=cat&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&exclude=' . $exclude . '&selected=' . $selected );
                                    } else if ( $cat_type == 'ajax' ) {
                                        wp_dropdown_categories( 'show_option_none=' . __( '-- Выберите категорию --', 'wpuf' ) . '&hierarchical=1&hide_empty=0&orderby=name&name=category1[]&id=cat-ajax&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&depth=1&exclude=' . $exclude . '&selected=' . $selected );
                                    } else {
                    ...
                    <li>
                        ...
                        if ( $editor == 'full' ) {
                            ...
                            <div style="float:left;">
                                <?php wp_editor( $curpost->post_content, 'new-post-desc1', array('textarea_name' => 'wpuf_post_content1', 'editor_class' => 'requiredField', 'teeny' => false, 'textarea_rows' => 8) ); ?>
                            </div>
                        <?php } else if ( $editor == 'rich' ) { ?>
                            <div style="float:left;">
                                <?php wp_editor( $curpost->post_content, 'new-post-desc1', array('textarea_name' => 'wpuf_post_content1', 'editor_class' => 'requiredField', 'teeny' => true, 'textarea_rows' => 8) ); ?>
                            </div>

                        <?php } else { ?>
                            <textarea name="wpuf_post_content1" class="requiredField" id="new-post-desc1" cols="60" rows="8"><?php echo esc_textarea( $curpost->post_content ); ?></textarea>
                        <?php } ?>
                    ...
                    <?php if ( wpuf_get_option( 'allow_tags', 'wpuf_frontend_posting' ) == 'on' ) { ?>
                        <li>
                            ...
                            <input type="text" name="wpuf_post_tags1" id="new-post-tags1" value="<?php echo $tagslist; ?>">
                    ...
                    <li>
                        <label>&nbsp;</label>
                        <input class="wpuf_submit" type="submit" name="wpuf_edit_post_submit1" value="<?php echo esc_attr( wpuf_get_option( 'update_label', 'wpuf_labels', 'Update Post' ) ); ?>">
                        <input type="hidden" name="wpuf_edit_post_submit1" value="yes" />
                        <input type="hidden" name="post_id1" value="<?php echo $curpost->ID; ?>">
                    </li>
                </ul>
            </form>
        </div>
    ...
    function submit_post() {
        ...
        if ( wpuf_get_option( 'allow_cats', 'wpuf_frontend_posting', 'on' ) == 'on' ) {
            ...
            } else if ( $cat_type == 'normal' && $_POST['category1'][0] == '-1' ) {
                $errors[] = __( 'Please choose a category', 'wpuf' );
            } else {
                if ( count( $_POST['category1'] ) < 1 ) {
                    $errors[] = __( 'Please choose a category', 'wpuf' );
                }
            }
        }
        ...
        if ( !$errors ) {
            ...
            if ( $post_id ) {
                echo '<div class="success">' . __( 'Статья успешна обновлена.', 'wpuf' ) . '</div>';
